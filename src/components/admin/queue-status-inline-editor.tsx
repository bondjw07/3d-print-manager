...
import { humanizeEnum, queueStatusOptions } from "@/lib/domain";
import { updateQueueItemAction } from "@/server/actions/portal-actions";
import type { QueuePriority, QueueStatus } from "@/generated/prisma/enums";

type QueueStatusInlineEditorProps = {
  queueItemId: string;
...
export function QueueStatusInlineEditor({
  queueItemId,
  currentStatus,
  priority,
  notes,
  redirectTo,
}: QueueStatusInlineEditorProps) {
  const [isEditing, setIsEditing] = useState(false);
  const [draftStatus, setDraftStatus] = useState<QueueStatus>(currentStatus);
  ...
  return (
    <form action={updateQueueItemAction} className="inline-flex items-center gap-1">
      ...
      <Select
        name="status"
        value={draftStatus}
        onChange={(event) => setDraftStatus(event.target.value as QueueStatus)}
      ...
      <button
        type="button"
        aria-label="Cancel queue status edit"
        title="Cancel"
        onClick={cancelEdit}
