      ...
      return <article key={product.id} className={`rounded-2xl border p-4 ${selection.selected ? "border-sky-400 bg-sky-50/40 dark:border-sky-700 dark:bg-sky-950/20" : "border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900"}`}>
        <div className="flex gap-3">
          ...
          <div className="min-w-0 flex-1 space-y-3">
            <div className="flex flex-wrap items-start justify-between gap-2"><div><h3 className="font-semibold text-slate-900 dark:text-slate-100">{product.publicName}</h3><p className="text-xs text-slate-500">{product.sku} · {product.category}</p></div><div className="grid gap-1"><label className="grid gap-1 text-sm font-medium text-slate-700 dark:text-slate-200">Price<input type="number" min="0.01" step="0.01" value={selection.price} onChange={(event) => update(product.id, { price: event.target.value })} disabled={!selection.selected} placeholder="0.00" className="h-9 w-28 rounded-xl border border-slate-300 bg-white px-2 text-sm disabled:opacity-50 dark:border-slate-700 dark:bg-slate-950" required={selection.selected} /></label><p className="text-xs text-slate-500">{product.suggestedCost === null ? "No material cost estimate" : `Estimated material cost: ${new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format(product.suggestedCost)}`}</p></div></div>
