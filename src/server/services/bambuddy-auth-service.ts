...
function encryptionKey() {
  ...
  if (!secret) throw new Error("APP_ENCRYPTION_KEY is not configured. Set it before saving the BamBuddy API key.");
...
function decrypt(payload: string) {
  ...
  if (version !== "v1" || !iv || !tag || !encrypted) throw new Error("Invalid encrypted BamBuddy API key.");
